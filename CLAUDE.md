...
Use yt-dlp to extract auto-generated transcripts. Check official Fed and institutional channels (Brookings, CFR, Peterson Institute) regularly; search media channels (CNBC, Bloomberg, WSJ) by speaker name and date when the official site has no record.
...
### Type D: FOMC Minutes
Released ~3 weeks after the meeting. Contains information not in the statement. Score independently. The primary focus is on **policymakers' views** — i.e., the "Participants' Views on Current Conditions and the Economic Outlook" section and the "Committee Policy Actions" discussion. The staff review sections (economic situation, financial situation, economic outlook) provide useful context but are secondary signals.

**Three comparisons are required when scoring minutes:**
1. **vs. Corresponding press conference opening statement** — the Chair's opening statement in the press conference is the committee's collective communication attempt (approved language, not just the Chair's personal views). This is a richer benchmark than the terse FOMC statement. Compare how the minutes' participant characterizations align with or diverge from the opening statement language.
2. **vs. Prior minutes** — the primary tool for tracking how collective committee views have shifted across meetings. Look for changes in the distribution of views (e.g., "several" → "many," "a few" → "several"), new themes not present in the prior minutes, and themes that disappeared.
3. **vs. Corresponding FOMC statement** — useful for identifying what the committee chose not to publicize. The statement is intentionally brief; significant committee views that did not make it into the statement are high-signal divergences.

**Neutral rate / restrictiveness language — precision matters:** Minutes language like "within the range of plausible estimates of its neutral level" is deliberately vague and may encompass participants who think policy is slightly above, at, or even slightly below neutral. Do not interpret this as definitively establishing policy is at neutral. Note the range of views explicitly expressed and flag what the phrase does and does not resolve.
...
2. **Read the rate path against the current FFR midpoint, not in isolation.** A median dot above the current midpoint implies hikes; below implies cuts. Report the directional split — how many participants are above / at / below the current rate — for the current year, and compare that split to the prior SEP. A flip in the modal direction (cut → hold → hike) is the headline.
3. **Longer-run rate vs. near-term path — restrictiveness, not neutral.** If the near-term median rises but the longer-run (neutral) median is unchanged, the Committee is signaling higher-for-longer relative to an unchanged neutral, NOT a re-estimate of neutral. Report the near-term-minus-longer-run gap for both SEPs as the measure of intended restrictiveness. (Same precision discipline as the Type D neutral-rate rule.)
4. **Dot distribution, not just the median.** Lay out the full dot histogram for the current year for both SEPs. Note tail collapse/expansion (e.g. a dovish tail disappearing) — the median can move modestly while the distribution shifts wholesale, or vice versa.
5. **Participant count and composition.** State the participant count for both SEPs and account for any change. Distinguish a NET change in count (a seat vacant / not submitting) from a COMPOSITION swap (one official replaced by another of different lean). **Do not over-attribute a median shift to a composition change** — the median is set by the middle dots, far from the tail; verify how many participants actually moved their own dots before crediting composition. Label all roster inferences with `[Inference]` and flag for analyst confirmation.
6. **Evaluate against the speeches since the prior meeting.** The SEP either confirms or contradicts the inter-meeting speech drift. Tie the dot move to specific officials' language and dissents (the dots formalize what the speeches signaled). Then flag any **dots-vs-statement gap** — individual positioning in the dots that has NOT yet consolidated into the statement language is a bottom-up leading indicator (see the Language Cycle).

**Historical archive:** every SEP since October 2007 is in `data/raw/sep/` (see its `README.md`), with a parsed dataset in `data/raw/sep/dataset/` (`sep_table1.csv`, `sep_dots_long.csv`, `sep_dots_summary.csv`). Run `python3 collect_sep_historical.py` then `python3 scripts/build_sep_dataset.py` after each projection meeting to extend it.
...
If multiple speakers appear, add a CROSS-SPEAKER NOTE before the UPCOMING section when their language on the same day is worth comparing.
...
────────────────────────────────────────────────────────────────
  ...
  [Host / Venue], [Location or "virtual"]

  (None confirmed) — [note if blackout begins]
════════════════════════════════════════════════════════════════
```
...
- **Post-FOMC meeting**: Immediately process statement diff; **on a projection meeting (March / June / September / December), also process the SEP / dot plot (Type I) the same day** — diff vs. the prior quarterly SEP, evaluate against the inter-meeting speeches, write the alert to `data/reports/fomc/` (emailed on push), and fold it into the next morning's canonical report; schedule minutes processing for ~3 weeks out; **update the reference FOMC date in CLAUDE.md and in all subsequent scored files and daily reports** — the reference date must always reflect the most recent FOMC meeting. Current reference FOMC: **September 16, 2026** (HIKE +25bp to 3.75–4.00%, 12–0 — the first increase since July 26, 2023; cycle regime now HIKING. Statement rewritten: the supply-shock attribution clause was DELETED from the inflation paragraph and a purpose clause added — "Today's policy action will support a timelier return to the Committee's 2 percent goal"; "domestic spending has been resilient" added; labor sentence unchanged for a third statement. SEP: 2026 median dot 4.1 (exact 4.125) vs. the new 3.875 midpoint — 16 of 18 participants above it, 0 below; 2027 median also 4.1; longer run 3.1 → 3.2; inflation medians +0.1/0/+0.1 while the rate path moved +0.3/+0.5/+0.5 — the labor/growth risk assessments flipped, not the inflation forecast. Warsh withheld his dot again (18 = 19 minus Warsh). See `data/reports/fomc/20260916_statement_diff.txt`, `data/scored/sep/20260916_SEP_scored.txt`, `data/reports/fomc/20260916_pressconf.txt`). Prior reference: July 29, 2026 (HOLD 3.50–3.75%, 9–3 with Hammack, Kashkari and Logan dissenting for a 25bp hike; text frozen vs. June 17 — `data/reports/fomc/20260729_statement_diff.txt`).
