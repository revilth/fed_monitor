...
- **Post-FOMC meeting**: Immediately process statement diff; **on a projection meeting (March / June / September / December), also process the SEP / dot plot (Type I) the same day** — diff vs. the prior quarterly SEP, evaluate against the inter-meeting speeches, write the alert to `data/reports/fomc/` (emailed on push), and fold it into the next morning's canonical report; schedule minutes processing for ~3 weeks out; **update the reference FOMC date in CLAUDE.md and in all subsequent scored files and daily reports** — the reference date must always reflect the most recent FOMC meeting. Current reference FOMC: **June 17, 2026** (statement HOLD 3.50–3.75%, easing bias dropped, unanimous; SEP turned decisively hawkish — see `data/scored/sep/20260617_SEP_scored.txt`).
