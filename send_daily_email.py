...
def find_weekend_reports() -> list[Path]:
    """Saturday and Sunday reports, for the Monday 6am catch-up email."""
    today = date.today()
    saturday = (today - timedelta(days=2)).strftime("%Y%m%d")
    sunday = (today - timedelta(days=1)).strftime("%Y%m%d")
    reports = []
    for d in [saturday, sunday]:
        reports.extend(sorted(REPORTS_DIR.glob(f"{d}*.txt")))
    return reports


def build_subject_and_body(reports: list[Path], weekend: bool = False) -> tuple[str, str]:
    if weekend:
        today = date.today()
        sat = (today - timedelta(days=2)).strftime("%b %d")
        sun = (today - timedelta(days=1)).strftime("%b %d, %Y")
        date_range = f"{sat}–{sun}"
        label = f"Weekend Catch-up — {date_range}"
    else:
        label = date.today().strftime("%B %d, %Y")

    if not reports:
        subject = f"Fed Monitor — {label} — No new reports"
        body = f"Fed Monitor — {label}\n\nNo new speeches scored over the weekend.\n" if weekend else f"Fed Monitor — {label}\n\nNo new speeches scored today.\n"
        return subject, body
    ...
    subject = f"Fed Monitor — {label} — {', '.join(names)}"
...
def main():
    ...
    if is_monday_morning():
        logger.info("Monday morning run — checking for weekend reports.")
        reports = find_weekend_reports()
        if not reports:
            logger.info("No weekend reports found — skipping email.")
            sys.exit(0)
        logger.info(f"Found {len(reports)} weekend report(s): {[r.name for r in reports]}")
        subject, body = build_subject_and_body(reports, weekend=True)
    else:
        if is_blackout_day():
        ...
        subject, body = build_subject_and_body(reports, weekend=False)
